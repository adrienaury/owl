...
// initUnitCommand initialize the cli append unit command
func initUnitCommand(parentCmd *cobra.Command) {
	cmd := &cobra.Command{
		Use:     "unit",
		Short:   "Append attributes to existing units",
		Long:    "",
		Aliases: []string{"units"},
		Example: fmt.Sprintf(`  %[1]s append unit <<< '{"ID": "my-unit", "Description": "Test unit"}'`, parentCmd.Root().Name()),
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for _, u := range units {
				err := unitDriver.Append(unit.NewUnit(u.ID, u.Description), policy.Objects()["unit"])
				if err != nil {
					cmd.PrintErrln(err)
					os.Exit(1)
				}
