...
func updateUnits(cmd *cobra.Command, units []exportedUnit, realmID string, p policy.Policy) {
	for _, u := range units {
		err := unitDriver.Update(unit.NewUnit(u.ID, u.Description), p.Objects()["unit"])
		if err != nil {
			cmd.PrintErrln(err)
			os.Exit(1)
		}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err == nil && len(format1.Units) > 0 {
				updateUnits(cmd, format1.Units, realmID, policy)
				return
			}
