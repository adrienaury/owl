...
func removeUnits(cmd *cobra.Command, units []exportedUnit, realmID string, p policy.Policy) {
	for _, u := range units {
		err := unitDriver.Remove(unit.NewUnit(u.ID, u.Description), p.Objects()["unit"])
		if err != nil {
			cmd.PrintErrln(err)
			os.Exit(1)
		}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err == nil && len(format1.Units) > 0 {
				removeUnits(cmd, format1.Units, realmID, policy)
				return
			}
