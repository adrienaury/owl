...
func importGroups(cmd *cobra.Command, groups []exportedGroup, realmID, unitID string) {
	...
	for _, g := range groups {
		...
		if modified {
			cmd.PrintErrf("Replaced group '%v' in unit '%v' of realm '%v'.", g.ID, unitID, realmID)
		} else {
...
func importUsers(cmd *cobra.Command, users []exportedUser, realmID, unitID string) {
	...
	for _, u := range users {
		...
		if modified {
			cmd.PrintErrf("Replaced user '%v' in unit '%v' of realm '%v'.", u.ID, unitID, realmID)
		} else {
...
func importUnits(cmd *cobra.Command, units []exportedUnit, realmID string, p policy.Policy) {
	for _, u := range units {
		modified, err := unitDriver.Apply(unit.NewUnit(u.ID, u.Description), p.Objects()["unit"])
		if err != nil {
			cmd.PrintErrln(err)
			os.Exit(1)
		}
		if modified {
			cmd.PrintErrf("Replaced unit '%v' in realm '%v'.", u.ID, realmID)
		} else {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if err == nil && len(format1.Units) > 0 {
				importUnits(cmd, format1.Units, realmID, policy)
				return
			}
